...
export default function OrbitImages({
  ...
  const containerHeight = (typeof height === 'number' || typeof height === 'string')
    ? height
    : (responsive ? 'auto' : (typeof width === 'number' ? width : 'auto'));

  const items = images.map((srcOrElem, index) => {
    ...
    return (
      <div key={index} className="orbit-custom-element w-full h-full">
        {srcOrElem}
      </div>
    );
  });
  ...
  return (
    <div
      ...
      style={{
        ...
        aspectRatio: responsive && !height ? '1 / 1' : undefined,
      }}
      aria-hidden="true"
    >
      <div
